...
!(function (t, e) {
	...
	class s {
		constructor(t, e, s) {
			(this.i = t),
				(this.u = '//app.finsc.vn/api/v1'),
				(this.m = 'https://histdatafeed.vps.com.vn/tradingview'),
				(this.p = e),
				(this._ = s),
				(this.N = 0);
		}
		getBars(t, s, i) {
			...
			let r = ['VNINDEX', 'VN30', 'HNX', 'HNX30'].includes(n.symbol)
					...
					? 'derivative'
					: 'stock',
				l = 'history' === r ? this.m : this.u;
			return (
				t.ticker.includes('#') && ((l = this.i), (r = 'history')),
				'history' === r && (n.resolution = s),
				void 0 !== i.countBack && (n.countback = i.countBack),
				void 0 !== t.currency_code && (n.currencyCode = t.currency_code),
				void 0 !== t.unit_id && (n.unitId = t.unit_id),
				new Promise(async (t, s) => {
					try {
						const e = await this.p.sendRequest(l, r, n),
							s = this.F(e, n);
						this._ && (await this.S(s, n)), t(s);
					} catch (t) {
		...
		async S(t, s) {
			...
			try {
				for (
					;
					this._ &&
					this._.maxResponseLength > 0 &&
					this._.maxResponseLength === i &&
					s.from < s.to;

				) {
					s.countback && (s.countback = s.countback - i),
						'earliestFirst' === this._.expectedOrder
							? (s.from = Math.round(t.bars[t.bars.length - 1].time / 1e3))
							: (s.to = Math.round(t.bars[0].time / 1e3));
					const e = await this.p.sendRequest(urlX, type, s),
						o = this.F(e);
					(i = o.bars.length),
						'earliestFirst' === this._.expectedOrder
		...
		F(t, e) {
			const s = [],
				i = { noData: !1 };
			if ('no_data' === t.s || 0 === t.t.length || null === t.t.length)
				(i.noData = !0),
					(i.nextTime =
						this.N <= 4 &&
						(t.nextTime
	...
	const i = new Map(),
		o = `finsc-${Math.floor(5e3 * Math.random())}`,
		...
		l = new Map(),
		a = mqtt.connect('wss://datafeed-lts.dnse.com.vn', r);
	function c(t) {
	...
	a.on('error', function (t) {
		...
		a.on('message', function (t, e) {
			...
			const m = `${c(h)}~${h}~${{ undefined: 1, HOUR1: '1H', DAY: '1D', W: '1W' }[d] || d}`,
				p = i.get(m);
			if (!p) return;
			let v = { time: 1e3 * o, open: n, high: r, low: l, close: a, volume: u };
			(p.lastDailyBar = v), p.handlers.forEach((t) => t.callback(v));
		});
	class u {
		constructor(t) {
			this.$ = t;
		...
		subscribeBars(t, e, s, o, n) {
			...
			!(function (t, e, s, o, n, r) {
				...
				(d = { listenerGuid: o, resolution: e, lastDailyBar: r, handlers: [h] }),
					i.set(u, d);
				const m = `plaintext/quotes/${l}/OHLC/${e}/${t.ticker}`;
				a.subscribe(m);
			})(t, d, s, (o = `${r}_#_${d}`), 0, l.get(t.symbol));
		}
		unsubscribeBars(t) {
	...
	class h {
		constructor(t, e) {
			(this.k = {}), (this.D = 0), (this.$ = t), setInterval(this.M.bind(this), e);
		}
		subscribeBars(t, e, s, i) {
			this.k.hasOwnProperty(i) ||
				((this.k[i] = { lastBarTime: null, listener: s, resolution: e, symbolInfo: t }),
				t.name);
		}
		unsubscribeBars(t) {
			delete this.k[t];
		...
		H(t) {
			...
			return this.$.getBars(e.symbolInfo, e.resolution, {
				from: i,
				to: s,
				countBack: 2,
				firstDataRequest: !1,
		...
		O(t, e) {
			if (!this.k.hasOwnProperty(t)) return;
			const s = e.bars;
	...
	class p {
		...
		getMarks(t, s, i, o, n) {
			if (!this.I.supports_marks) return;
					...
					if (!Array.isArray(t)) {
						...
						for (let s = 0; s < t.id.length; ++s)
							e.push({
								...
								text: d(t, 'text', s),
								label: d(t, 'label', s),
								labelFontColor: d(t, 'labelFontColor', s),
								minSize: d(t, 'minSize', s),
								borderWidth: d(t, 'borderWidth', s),
								hoveredBorderWidth: d(t, 'hoveredBorderWidth', s),
								imageUrl: d(t, 'imageUrl', s),
								showLabelWhenImageLoaded: d(t, 'showLabelWhenImageLoaded', s),
							});
						t = e;
					}
					o(t);
		...
		getTimescaleMarks(t, s, i, o, n) {
			if (!this.I.supports_timescale_marks) return;
			const r = { symbol: t.ticker || '', from: s, to: i, resolution: n };
		...
		async U() {
			const t = await fetch('/static/tv/dist/symbol.json'),
				e = await t.json();
			this.R = e;
		}
		async searchSymbols(t, s, i, o) {
			if (this.I.supports_search) {
		...
		async resolveSymbol(t, s, i, o) {
			...
			if (this.I.supports_group_request) {
				const e = t.split('#')[0],
				...
				l({
					...
					supported_resolutions: this.I.supported_resolutions,
					intraday_multipliers: this.I.intraday_multipliers,
					volume_precision: 2,
									...
									x = {
										...
										unit_id:
											null !== (r = t.unit_id) && void 0 !== r
												...
												: t['unit-id'],
										original_unit_id:
											null !== (a = t.original_unit_id) && void 0 !== a
												? a
												: t['original-unit-id'],
										...
										has_weekly_and_monthly:
											null !== ($ = t.has_weekly_and_monthly) && void 0 !== $
												? $
												: t['has-weekly-and-monthly'],
										has_empty_bars:
											null !== (k = t.has_empty_bars) && void 0 !== k
												? k
												: t['has-empty-bars'],
										volume_precision:
											null !== (D = t.volume_precision) && void 0 !== D
												? D
												: t['volume-precision'],
										format:
											null !== (M = t.format) && void 0 !== M ? M : 'price',
									};
								(x.supported_resolutions = [
									'1',
									'3',
									'5',
									'15',
		...
		subscribeBars(t, e, s, i, o) {
			'index' === this.q(t.ticker)
				? this.B.subscribeBars(t, e, s, i)
				: this.C.subscribeBars(t, e, s, i);
		}
		unsubscribeBars(t) {
			this.B.unsubscribeBars(t), this.C.unsubscribeBars(t);
		}
		A() {
		...
		X(t, e) {
			return this.p.sendRequest(this.T, t, e);
		}
		L(t, e) {
			return this.p.sendRequestPost(this.T, t, e);
		}
		P(t) {
			if (
				((this.I = t),
			...
			(!t.supports_group_request && t.supports_search) ||
				(this.V = new SymbolsStorage(this.T, t.supported_resolutions || [], this.p)),
				JSON.stringify(t);
		}
	}
	class v {
